...
namespace TacticalAI
{
    public class BulletScript : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            switch(LevelSelectionNew.modeSelection)
            {
                case LevelSelectionNew.modeType.ASSAULT:
                    //damage = GameManager.Instance.damageFactor;
                    damage = /*0.005f + */((PlayerPrefs.GetInt("LevelCount") / (70 - PlayerPrefs.GetFloat("Difficulty"))));
                    break;
                case LevelSelectionNew.modeType.BR:
                case LevelSelectionNew.modeType.FREEFORALL:
                    damage = 5 + (PlayerPrefs.GetInt("LevelCount") / 5);
                    break;
                case LevelSelectionNew.modeType.MULTIPLAYER:
                    damage = 4 + (PlayerPrefs.GetInt("LevelCount") / 5);
                    break;
            }
        ...
        IEnumerator ApplyDamage()
        {
                    ...
                    }else if(hit.collider.GetComponent<HitBox>() && teamID != hit.collider.GetComponent<HitBox>().teamID)
                    {
#if UNITY_EDITOR
                        Debug.Log("damaging ai ally");
#endif
                        hit.collider.GetComponent<HitBox>().myScript.Damage(damage, hit);
            ...
            //    if (hit.transform.tag != friendlyTag)
            //    {
            //        if (MultiPlayerMenu.isMulti && hit.collider.gameObject.GetComponent<FPSPlayer>())
            //        {
            //            hit.collider.gameObject.GetComponent<FPSPlayer>().ApplyDamage(damage, healthScript, teamID, statIndex, nameEnemies, kills, death, score, gunID);
            //        }
            //        else if (hit.collider.gameObject.GetComponent<FPSPlayer>())
            //        {
            //            hit.collider.gameObject.GetComponent<FPSPlayer>().ApplyDamage(damage);
            //        }
            //        if (MultiPlayerMenu.isMulti && hit.collider.gameObject.GetComponent<HitBox>())
            //        {
            //            hit.collider.GetComponent<HitBox>().Damage(healthScript, damage, teamID, statIndex, nameEnemies, kills, death, score, gunID);
            //        }
            //        else
            //            hit.collider.SendMessage(damageMethodName, damage, SendMessageOptions.DontRequireReceiver);
            //    }
