...
namespace TacticalAI
{
    public class BaseScript : MonoBehaviour
    {
        ...
        //Status Stuff
        public bool engaging, canEngage = true;
        ...
        void Awake()
        {
            levelIndex = PlayerPrefs.GetInt("sniperCurrentLevel");
            ...
            switch (LevelSelectionNew.modeSelection)
            {
                case LevelSelectionNew.modeType.SNIPER:
                    myTargetScript.maxDistToNoticeTarget = 0;
                    break;
                case LevelSelectionNew.modeType.ASSAULT:
                    myTargetScript.maxDistToNoticeTarget = 999999;
                    canSprint = false;
                    isAssault = true;
                    break;
                case LevelSelectionNew.modeType.FREEFORALL:
                    canSprint = true;
                    myTargetScript.maxDistToNoticeTarget = 180;
                    wanderDiameter = 230;
                    break;
                case LevelSelectionNew.modeType.BR:
                    ...
                    {
                        myTargetScript.maxDistToNoticeTarget = 230;
                        isCamper = true;
                    }
                    else
                    {
                        myTargetScript.maxDistToNoticeTarget = 175;
                    }
            ...
            /*if(!MultiPlayerMenu.isMulti && LevelSelectionNew.modeSelection == LevelSelectionNew.modeType.SNIPER)
            {
                switch (levelIndex)
                {
                    case 1:
                        myTargetScript.maxDistToNoticeTarget = 5;
                        myIdleBehaviour = IdleBehaviour.Patrol;
                        patrolNodes[0] = this.transform;
                        break;
                    case 2:
                        myTargetScript.maxDistToNoticeTarget = 5;
                        myIdleBehaviour = IdleBehaviour.Patrol;
                        ...
                        break;
                    case 3:
                        if (this.gameObject.name.Equals("Sniper_Idle(Clone)"))
                        {
                            myIdleBehaviour = IdleBehaviour.Patrol;
                            patrolNodes[0] = this.transform;
                        }
        ...
        void Start()
        {
            ...
            {
                //if (GameManager.Instance.assaultLevels[11].gameObject.activeInHierarchy)
                   // gameObject.GetComponent<NavMeshAgent>().stoppingDistance = -0.3f;

                Invoke("DelayedDistanceAllow", 1.5f);
            } 
        }
        ...
        public void SetCombatBehaviour(CustomAIBehaviour c)
        {
            ...
            if (canEngage && canOverrideBehaviour)
                SetBehaviour();
        }
        ...
        bool allowDistanceCheck = false;

        void DelayedDistanceAllow()
        {
            allowDistanceCheck = true;
        }
        ...
        void MoveAI()
        {
            switch(LevelSelectionNew.modeSelection)
            {
                case LevelSelectionNew.modeType.ASSAULT:
                    ...
                    {
                        if (allowDistanceCheck && myTargetScript.currentDistance < 200)
                        {
                            navI.SetDestination(transform.position);
                        }
                        else
                        {
                            navI.SetDestination(TargetScript.commonTarget.transform.position);
                        }
                    }
                    else
                    {
        ...
        {
            if (canEngage && canOverrideBehaviour && shouldRunFromGrenades)
            {
        ...
        public bool SetDynamicObject(Transform newMovementObjectTransform, string anim, string newMethodToCall, bool requireEngaging, float timeToWait)
        {
            ...
            if (!isDodging && (!requireEngaging || engaging) && canEngage && canOverrideBehaviour && canUseDynamicObject)
            {
        ...
        public void KillAI()
        {
            ...
            if (this.enabled)
            {
                GameManager.firstBulletDelegate -= Retaliate;
                GameManager.firstBulletDelegate -= Runaway;
                GameManager.firstBulletDelegate -= AlertAnim;

                GameManager.secondBulletDelegate -= Retaliate;
                GameManager.secondBulletDelegate -= Runaway;
                GameManager.secondBulletDelegate -= AlertAnim;
                if (combatBehaviour)
        ...
        {
            //Waqas optimization Steps (remove isimpleAi check)
            if(ControllerScript.isSimpleAI)
            {
            ...
            else if (!isDodging && !isMeleeing && !inParkour && !isStaggered && !navI.OnNavmeshLink() && (!useAdvancedCover || !inCover))
            {
                grenadeSpawn = spawnPos;
                targetPos = tPos;
                grenadePrefab = prefab;
        ...
        {
            isStatic = true;
            animationScript.PlaySitting();
        }
        public void AlertAnim()
        {
            animationScript.PlayAlert();
        }
        public void Retaliate()
        {
            ...
            {
                CancelInvoke();
                currentBehaviour = combatBehaviour;
                isStatic = true;
                canSprint = false;
        ...
        public void SetMoveWithDelayedRetaliate(Transform t,float time)
        {
            isStatic = false;
            myTargetScript.maxDistToNoticeTarget = 0.1f;
            keyTransform = t;
            patrolNodes = new Transform[1];
            patrolNodes[0] = t;
            targetTransform = t;
            headLookScript.SetTargetTransform(targetTransform);
            Invoke("Retaliate", time);
        }
        public void SetMoveTarget(Transform t)
        {
            isStatic = false;
            canEngage = false;
            myTargetScript.maxDistToNoticeTarget = 0f;
            targetTransform = t;
            keyTransform = t;
            headLookScript.SetTargetTransform(targetTransform);
        }
        public void SetRunTarget(Transform t)
        {
            idleSpeed *= 2;
            runSpeed *= 2;
            sprintSpeed *= 2;
        ...
        public void Runaway()
        {
            ...
            canEngage = false;
            myTargetScript.maxDistToNoticeTarget = 0f;
            keyTransform = escapeLocation;
            //animationScript.StopSitting();
            headLookScript.SetTargetTransform(escapeLocation);
            animationScript.ForceSprinting();
#if UNITY_EDITOR
            print("WahabBro");
#endif
            //Invoke("DelayedForceEnd", 14.5f);
            if(LevelSelectionNew.modeSelection == LevelSelectionNew.modeType.SNIPER)
            {
                if(PlayerPrefs.GetInt("sniperCurrentLevel") == 13)
                {
#if UNITY_EDITOR
                    Invoke("DelayedForceEnd", 20.0f);
#endif
                }
                else
                {
        ...
        void DelayedForceEnd()
        {
            GameManager.Instance.ForceLevelFail();
        }
        private void OnDisable()
        {
            CancelInvoke("DelayedDistanceAllow");
            CancelInvoke("Retaliate");
            CancelInvoke("DelayedForceEnd");
            StopCoroutine("AICycle");
            StopCoroutine("PerformanceAICycle");
        }
