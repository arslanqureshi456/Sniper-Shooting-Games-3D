...
namespace TacticalAI
{
    public class TargetScript : MonoBehaviour
    {
        ...
        bool canEngage = true;
        public static TacticalAI.Target commonTarget;
        public bool isShootingScene = false;
        ...
        void Awake()
        {
            ...
            {
                commonTarget = new TacticalAI.Target(0, myUniqueID, targetObjectTransform, this);
            }
            ...
            //{
            //    StartCoroutine(IncreaseMaxDistToNoticeTarget());
            //}
        }
        //ABDUL
        private IEnumerator IncreaseMaxDistToNoticeTarget()
        {
            if (!isShootingScene)
            {
                yield return new WaitForSeconds(5f);
            }
            maxDistToNoticeTarget = 120;
        }
            ...
            while (myAIBaseScript.isCurrentlyActive())
            {
                ...
                switch(LevelSelectionNew.modeSelection)
                {
                    case LevelSelectionNew.modeType.ASSAULT:
                        currentDistance = Vector3.SqrMagnitude(eyeTransform.position - commonTarget.transform.position);
                        break;
                }
        ...
        {
            if (!ControllerScript.isSimpleAI && TacticalAI.ControllerScript.currentController != null && isPlaying)
        ...
        public void UpdateEnemyAndAllyLists(TacticalAI.Target[] e)
        {
            if (!ControllerScript.isSimpleAI)
            {
                if (myAIBaseScript)
                {
                    ...
                    for (int i = 0; i < lastTargets.Length; i++)
                    {
                        ...
                        {
                            if (lastTargets[i].uniqueIdentifier == enemyTargets[x].uniqueIdentifier && lastTargets[i].uniqueIdentifier != myUniqueID)
                            {
            ...
            switch(LevelSelectionNew.modeSelection)
            {
               default:
                    if (eyeTransform)
                    {
                        ...
                        for (i = 0; i < listOfCurrentlyNoticedTargets.Count; i++)
                        {
                            if (listOfCurrentlyNoticedTargets[i].transform)
                            {
                                ...
                                if (!Physics.Linecast(eyeTransform.position, enemyTransformCheckingNow.position, layerMask))
                                {
                                    ...
                                    if ((LevelSelectionNew.modeSelection == LevelSelectionNew.modeType.BR || LevelSelectionNew.modeSelection == LevelSelectionNew.modeType.FREEFORALL) && enemyScoreCheckingNow < 4)
                                        enemyScoreCheckingNow = 0;
                        ...
                        if(currentEnemyTarget != null && currentEnemyTarget.transform && currentEnemyTarget.transform.gameObject == transform.gameObject)
                        {
                    ...
                    break;
                case LevelSelectionNew.modeType.ASSAULT:
                    if (commonTarget.transform != null && currentDistance > maxDistToNoticeTarget * 4.8f)
                    {
                        if (engaging)
                        {
                            myAIBaseScript.RemoveMyTarget();
        ...
        public void AlertAlliesOfEnemy_Shout()
        {
            ...
            if (!ControllerScript.isSimpleAI && currentEnemyTarget != null && currentEnemyTarget.transform)
            {
        ...
        {
            if (LevelSelectionNew.modeSelection == LevelSelectionNew.modeType.SNIPER || LevelSelectionNew.modeSelection == LevelSelectionNew.modeType.FREEFORALL)
            {
                shouldCheck360Degrees = true;
            }
            switch(LevelSelectionNew.modeSelection)
            {
                default:
                    if (enemyTargets != null)
                    {
                        for (int i = 0; i < enemyTargets.Length; i++)
                        {
                            ...
                            if (enemyTargets[i].uniqueIdentifier != myUniqueID && eyeTransform && enemyTargets[i].transform && (shouldCheck360Degrees || Vector3.Angle(eyeTransform.forward, enemyTargets[i].transform.position - eyeTransform.position) < effectiveFOV) && Vector3.SqrMagnitude(eyeTransform.position - enemyTargets[i].transform.position) < maxDistToNoticeTarget)
                            {
                                //(Vector3.Angle(eyeTransform.forward, enemyTargets[i].transform.position - eyeTransform.position));
                                //print(shouldCheck360Degrees);
                                if ((LevelSelectionNew.modeSelection == LevelSelectionNew.modeType.BR || LevelSelectionNew.modeSelection == LevelSelectionNew.modeType.FREEFORALL) && Vector3.SqrMagnitude(eyeTransform.position - enemyTargets[i].transform.position) < 4)
                                    return;
                    ...
                    break;
                case LevelSelectionNew.modeType.ASSAULT:
                    if (commonTarget.transform != null && currentDistance < maxDistToNoticeTarget)
                    {
                        if (canEngage && !engaging)
                        {
                            myAIBaseScript.SetMyTarget(commonTarget.transform, commonTarget.targetScript.myLOSTarget);
                            myAIBaseScript.StartEngage();
                            engaging = true;
                        }
                    }
            ...
            //}
            //else if (Vector3.SqrMagnitude(eyeTransform.position - commonTarget.transform.position) < maxDistToNoticeTarget)
            //{
            //    if (!engaging)
            //    {
            //        myAIBaseScript.SetMyTarget(commonTarget.transform, commonTarget.targetScript.myLOSTarget);
