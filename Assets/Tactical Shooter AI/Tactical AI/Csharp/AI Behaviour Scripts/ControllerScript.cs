...
namespace TacticalAI
{
    ...
    {
        public static bool isSimpleAI = false;
        ...
        public void UpdateAllEnemiesEnemyLists()
        {
            ...
            if(!isSimpleAI)
            {
                for (int y = 0; y < currentTargets.Count; y++)
                {
                    ...
                    {
                        currentTargets[y].targetScript.UpdateEnemyAndAllyLists(currentTargets.ToArray());
                    }
                        
                    else
                        currentTargets[y].targetScript.UpdateEnemyAndAllyLists(GetCurrentTargetsWithIDs(currentTargets[y].targetScript.enemyTeamsIDs));
                }
            }
        ...
        int closestIndex;
        public Target GetClosestTarget(Vector3 eye,float Distance)
        {
            distance = tempdistance = 9999999;
            for(int i=0;i<currentTargets.Count; i++)
            {
                if(currentTargets[i].transform != null)
                {
                    tempdistance = Vector3.SqrMagnitude(eye - currentTargets[i].transform.position);
                    if (tempdistance > 2f && tempdistance < Distance && tempdistance < distance)
                    {
...
#if UNITY_EDITOR
            Debug.Log("returning target " + closestIndex + " total " + currentTargets.Count);
#endif
            return currentTargets[closestIndex];
        }
