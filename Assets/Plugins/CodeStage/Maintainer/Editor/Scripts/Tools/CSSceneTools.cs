...
namespace CodeStage.Maintainer.Tools
{
	...
	internal static class CSSceneTools
	{
		...
		public static OpenSceneResult OpenScene(string path, bool activate = false)
		{
			...
			{
				Debug.LogError(Maintainer.ConstructErrorForSupport("Can't open scene since path is absent!"));
				return result;
			}
			...
			if (!targetScene.isLoaded)
			{
				...
				{
					Debug.LogError(Maintainer.ConstructErrorForSupport("Error while opening scene: " + path + "\n" + e));
					return result;
				}
				...
				{
					Debug.LogError(Maintainer.ConstructErrorForSupport("Can't open scene since path leads to invalid scene!"));
					return result;
				}
		...
		public static IEnumerable<string> GetAllScenes()
		{
			...
			foreach (var scene in allScenes)
			{
				...
				var kind = CSEditorTools.GetAssetKind(scenePath);
