...
public partial class M_Colors_Window : _W___IWindow {
    ...
    static void ListToTextColor( Adapter.TempColorClass list, out Color color, out bool hasColor )
    {   color = list.LABELCOLOR;
        hasColor = color[0] > 0 || color[1] > 0 || color[2] > 0 || color[3] > 0;
    }
    static void ListToBgColor( Adapter.TempColorClass list, out Color color, out bool hasColor )
    {   color = list.BGCOLOR;
        hasColor = color[0] > 0 || color[1] > 0 || color[2] > 0 || color[3] > 0;
    }
    static void ListToBgColor( SingleList list, out Color color, out bool hasColor )
    {   color = list.list.Count < 4 ? Adapter.TRANSP_COLOR : (Color)new Color32( (byte)list.list[0], (byte)list.list[1], (byte)list.list[2], (byte)list.list[3] );
        hasColor = color[0] > 0 || color[1] > 0 || color[2] > 0 || color[3] > 0;
    }
    /*   static   void ListToTextColor(SingleList list, out Color color, out bool hasColor)
       {   color = list.list.Count < 9 ? Adapter.TRANSP_COLOR : (Color)new Color32((byte)list.list[5], (byte)list.list[6], (byte)list.list[7], (byte)list.list[8]);
           hasColor = color[0] > 0 || color[1] > 0 || color[2] > 0 || color[3] > 0;
       }
       static  void ListToBgColor(SingleList list, out Color color, out bool hasColor)
       {   color =  list.list.Count < 4 ? Adapter.TRANSP_COLOR : (Color)new Color32((byte)list.list[0], (byte)list.list[1], (byte)list.list[2], (byte)list.list[3]);
    ...
    {   var getcolor = GetHiglightData();
        ...
        Adapter.LABEL( inputrect, "<i>Highlighter:</i>" );
        ...
        {   var rad = inputrect;
            ...
            if ( !(adapter.ColorModule.GET_CONTENT( source ).add_icon || Enable_BackgroundColor || LabelEnable) )
            {   rad.width *= 3;
                Adapter.LABEL( rad, "<i>Use ctrl click to close the window immediately</i>" );
            }
            ...
            Adapter.TOOLTIP( rad, "If the child is on, the highlighter settings will affect all children." );
            
            rad.x += rad.width * 2;
            GUI.enabled = o_n;
            
            if ( ( adapter.ColorModule.GET_CONTENT( source ).add_icon || Enable_BackgroundColor || LabelEnable) && GUI.Button( rad, "Clear", adapter.STYLE_DEFBUTTON_right ) )
        ...
        //* TEXT COLOR *//
        {
            ...
            //var  refCol = getcolor.list.Count < 9 ? Adapter.TRANSP_COLOR : (Color)new Color32((byte)getcolor.list[5], (byte)getcolor.list[6], (byte)getcolor.list[7], (byte)getcolor.list[8]);
            ...
            // if (en2)
            {
                ...
                var l = Hierarchy_GUI.GetLastHiglightTextList( adapter );
                
                //                     if ( Event.current.type == EventType.Repaint )
                //                         Adapter.STYLE_DEFBOX.Draw( new Rect( R_LAST.x , R_LAST.y , R_LAST.width , R_LAST.height ) , "" , false , false , false , false );
                /* if (LabelEnable)*/
        ...
        //* HIGLIGHTER */
        {
            ...
            //var refCol = getcolor.list.Count < 4 ? Adapter.TRANSP_COLOR : (Color)new Color32((byte)getcolor.list[0], (byte)getcolor.list[1], (byte)getcolor.list[2], (byte)getcolor.list[3]);
            ...
            {   Enable_BackgroundColor = newEn;
                ...
                {   /*var lastHiglight = Hierarchy_GUI.GetLastHiglightList( adapter );
                    ...
                    getcolor.BGCOLOR = getcolor.GetLastTempColor( adapter ).BGCOLOR;
            ...
            //if (en)
            {
                ...
                var l = Hierarchy_GUI.GetLastHiglightList( adapter );
                /*  if ( Event.current.type == EventType.Repaint )
                  {   var c = GUI.color;
                ...
                if ( Enable_BackgroundColor )
                {
                    ...
                    var r2 = r;
                    r2.width /= 2;
                    nv = adapter.TOGGLE_LEFT( r2, "Narrow Height", getcolor.BG_HEIGHT == 1, skipMark: true ) ? 1 : getcolor.BG_HEIGHT == 1 ? 0 : getcolor.BG_HEIGHT;
                    if ( nv != getcolor.BG_HEIGHT )
                    {   getcolor.BG_HEIGHT = nv;
                        SetHiglightData( getcolor, "Change Narrow Color" );
                    }
                    r2.x += r2.width;
                    nv = adapter.TOGGLE_LEFT( r2, "1 pixel Height", getcolor.BG_HEIGHT == 2, skipMark: true ) ? 2 : getcolor.BG_HEIGHT == 2 ? 0 : getcolor.BG_HEIGHT;
                    if ( nv != getcolor.BG_HEIGHT )
                    {   getcolor.BG_HEIGHT = nv;
                        SetHiglightData( getcolor, "Change Narrow Color" );
                    }
                    Adapter.TOOLTIP( r, "The size of the background will be calculated based on font size and not the height of the line." );
                    r.y += r.height + 2;
                    
                    
                    
                    RECT = r;
                    Adapter.LABEL( RECT, "<i>Horizontal Align:</i>" );
                    r.y += r.height;
                    
                    
                    RECT = r;
                    /* Adapter.LABEL(RECT, "<i>Left Align:</i>");
                     RECT.x += RECT.width / 5;
                    ...
                    RECT = r;
                    /*  Adapter.LABEL(RECT, "<i>Right Align:</i>");
                      RECT.x += RECT.width / 5;
                      RECT.width -= RECT.width / 5;*/
                    
                    var er = RECT;
                    er.width = er.width / 6;
                    nv = adapter.TOGGLE_LEFT( er, "Fixed", getcolor.BG_ALIGMENT_RIGHT == 5, skipMark: true ) ? 5 : 0;
                    ...
                    /*   EditorGUI.EnumPopup(r, (Adapter.BgAligmentLeft) getcolor.BG_ALIGMENT_LEFT);
                       r.y += r.height;
                       EditorGUI.EnumPopup(r, (Adapter.BgAligmentRight) getcolor.BG_ALIGMENT_RIGHT);
                       var o__n = GUI.enabled;
                       GUI.enabled = (Adapter.BgAligmentRight) getcolor.BG_ALIGMENT_RIGHT == Adapter.BgAligmentRight.WidthFixedGradient;
                       r.x += r.width;
                       EditorGUI.IntField(r, "Width:", getcolor.BG_WIDTH );
                       GUI.enabled = o__n;*/
                    /*  r.x += r.width;
                      r.x += r.width;
                      EditorGUI.EnumPopup(r, (Adapter.BgAligmentHeight) getcolor.BG_HEIGHT);*/
        ...
        {   var contC = GUI.color;
            ...
            DrawStroke(new Rect(R_CHILD.x + Adapter.GET_SKIN().button.padding.left, R_CHILD.y, R_CHILD.width, R_CHILD.height), "Apply\nTo Childes", GUI.color);
            GUI.color = contC;
        }*/
        // var newchild = child;
    ...
    void DRAW_LAST_HIGHLIGHTER( Rect inputrect, ref Color result, ref List<Color32> list, bool bg )
    {
        ...
        {   var c = GUI.color;
            ...
            //  Adapter.STYLE_DEFBOX.Draw( new Rect( inputrect.x, inputrect.y, inputrect.width, inputrect.height ), "", false, false, false, false );
            GUI.color = c;
        }
        ...
        // Adapter. INTERNAL_BOX(new Rect(0, 0, FULL_RECT.width + PICKER_RECT.width, FULL_RECT.height), "");
        GUI.color = asd;
