...
public partial class M_Colors_Window : _W___IWindow {
    ...
    {   inputrect.width -= 5;
        ...
        var im = adapter.ColorModule.GET_CONTENT(source).add_icon;
        if (im != null)
        {   GUI.DrawTexture( R_CLOSE, adapter.GetIcon( "HIPERUI_CLOSE" ) );
            ...
            adapter.ColorModule.CHeckIcon( source, im );
        }
        ...
        //* ICON */
        if (im != null && !adapter.DISABLE_DESCRIPTION( source ))
        {
        
        
            var getcolor = GetIconColor();
            refCol = (Color)new Color32( (byte)getcolor.list[0], (byte)getcolor.list[1], (byte)getcolor.list[2], (byte)getcolor.list[3] );
        ...
        {   //cats = new [] {  "External Texture2D Icons"};
            ...
            var ov = EditorPrefs.GetInt("EModules/Hierarchy/IconCat");
            ov = Mathf.Clamp(ov, 0, cats.Length - 1);
            nv = GUI.Toolbar(ll,   ov, cats, EditorStyles.toolbarButton);
            if (ov != nv)
            {   EditorPrefs.SetInt("EModules/Hierarchy/IconCat", nv);
            }
    ...
    {   /*  inputrect.x += 2;
        ...
        if (Adapter.M_Colors.labelIcons != null && Adapter.M_Colors.labelIcons.Length >= 8)
        {   // GUI.BeginVertical();//label
            ...
            {   //  GUI.BeginHorizontal();//label
                ...
                {   /*var bg = Adapter.GET_SKIN().box.normal.background;
                    ...
                    bs.normal.background = (Texture2D)Adapter.M_Colors.labelIcons[x + y * 4].image;
                    ...
                    ICON_RECT.y -= 1;
                    if (Button( ICON_RECT, "" ))
                    {   SetIconImage( Adapter.M_Colors.labelIcons[x + y * 4].image,  "Set Icon" );
                        REPAINT_ALL_HIERW();
                        //InternalEditorUtility.RepaintAllViews();
                        TryToClose();
                        #if CLOSE_AFTERICON
        ...
        if (Adapter.M_Colors.largeIcons != null && Adapter.M_Colors.largeIcons.Length >= 16)
        {   //   GROUP_RECT = new Rect(lastRect.x + 4 * w10 + PAD, lastRect.y, 4 * w10 + PAD, lastRect.height);
            ...
            {   for (int x = 0 ; x < 4 ; x++)
                {   /*var bg = Adapter.GET_SKIN().box.normal.background;
                    ...
                    GUI.DrawTexture( ICON_RECT, (Texture2D)Adapter.M_Colors.largeIcons[x + y * 4].image, ScaleMode.ScaleToFit );
                    ...
                    ICON_RECT.y -= 1;
                    if (Button( ICON_RECT, "" ))
                    {   SetIconImage( (Texture2D)Adapter.M_Colors.largeIcons[x + y * 4].image,  "Set Icon" );
                        REPAINT_ALL_HIERW();
                        // InternalEditorUtility.RepaintAllViews();
                        TryToClose();
                        #if CLOSE_AFTERICON
    ...
    void DRAW_LAST_ICONS(Rect inputrect)
    {
        ...
        for (int i = 0, Interator = 0 ; i < Math.Min( Hierarchy_GUI.GetLastList( adapter ).Count, LCOUNT ) && Interator < Hierarchy_GUI.GetLastList( adapter ).Count ; i++, Interator++)
        {   if (string.IsNullOrEmpty( Hierarchy_GUI.GetLastList( adapter )[Interator] )) continue;
            Texture2D display = null;
            if (Hierarchy_GUI.GetLastList( adapter )[Interator].StartsWith( "GUID=" ))
            {   var path = AssetDatabase.GUIDToAssetPath(Hierarchy_GUI.GetLastList( adapter )[Interator].Substring(5));
                if (path != null) display = AssetDatabase.LoadAssetAtPath<Texture2D>( path );
            }
            else
            {   Adapter.M_Colors.InitIcons();
                display = adapter.ColorModule.GetIcon( Hierarchy_GUI.GetLastList( adapter )[Interator] );
            }
