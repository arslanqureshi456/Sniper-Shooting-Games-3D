...
public partial class M_Colors_Window : _W___IWindow {
    ...
    {   Hierarchy_GUI.Undo( adapter, "Change Presets Data" );
        Hierarchy_GUI.Instance( adapter ).GETPRESETS_DATA = Adapter.SERIALIZE_SINGLE( cache );
        Hierarchy_GUI.SetDirtyObject( adapter );
        ClearCacheAndRepaint();
        foreach ( var focusRoot in __inputWindow )
    ...
    {   if ( cache == null )
        {   try
            {   cache = Adapter.DESERIALIZE_SINGLE<PresetsData>( Hierarchy_GUI.Instance( adapter ).GETPRESETS_DATA );
            }
            catch       // ignored
            {
            }
    ...
    {   var skin = Adapter.GET_SKIN();
        ...
        skin.verticalScrollbarThumb.normal.background = adapter.GetIcon( "HIPERUI_LINE_BOX" );
    ...
    void DRAWPRESETS( Rect inputrect )
    {
        ...
        {   if ( adapter.pluginID == 0 ) Selection.objects = new[] { (UnityEngine.Object)source.go };
            else Selection.objects = new[] { source.GetHardLoadObject() };
        }
        ...
        var ov = adapter.par.PresetManagerParams.SAVE_GAMEOBJEST ? 1 : 0;
        ...
        if ( nv != ov )
        {   adapter.par.PresetManagerParams.SAVE_GAMEOBJEST = nv == 1;
            adapter.SavePrefs();
        }
        ...
        var selected_index = Hierarchy_GUI.Instance(adapter).GETPRESETS_SELECT;
    ...
    void DRAW_CATEGORIES( Rect inputrect, int selected_index )
    {
        ...
        var scroll = new Vector2(0, Hierarchy_GUI.Instance(adapter).GETPRESETS_SCROLL_CAT);
        ...
        scroll = GUI.BeginScrollView( group_rect, scroll, rr, false, false );
        RESET_SCROLL();
    ...
    {   Label( r_line, title, TextAnchor.MiddleCenter );
        ...
        {   var button_rect = new Rect(r_line.x + 3, r_line.y + (r_line.height - 15) / 2, r_line.width - 6, 15);
            ...
            if ( i == selected_index ) GUI.DrawTexture( button_rect, adapter.GetIcon( "HIGHLIGHTER_PRESETS_SELECTION" ) );
            ...
            if ( categorie.categories[i].cache_IAMGE == null )
            {
                ...
                if ( select_adding != 0 )
                {   categorie.categories[i].cache_IAMGE = Utilities.ObjectContent_NoCacher( adapter, (UnityEngine.Object)null, typeof( Transform ) ).add_icon;
                }
                else if ( count != 0 )
                {   var t = Adapter.GET_TYPE_BY_STRING(categorie.categories[i].PRESET_COMPONENT_TYPE);
                    if ( t != null )
                    {   categorie.categories[i].cache_TYPE = t;
                        categorie.categories[i].cache_IAMGE = Utilities.ObjectContent_NoCacher( adapter, (UnityEngine.Object)null, t ).add_icon;
                        if ( !categorie.categories[i].cache_IAMGE ) categorie.categories[i].cache_IAMGE = adapter.GetIcon( "MONO" );
                    }
                }
            }
            ...
            else adapter.DRAW_SMALL_NUMBER( r_icon, "-" );
            
            adapter.DRAW_SMALL_NUMBER( numbers_rect, count < 1 ? "-" : count.ToString() );
            // DRAW_SMALL_NUMBER(new Rect(r_icon.x, r_icon.y, r_icon.width, r_icon.height), count < 1 ? "-" : count.ToString());
            //* ICON *//
            
            GUI.DrawTexture( new Rect( r_line.x + 9, r_line.y + r_line.height - 1, r_line.width - 18, 5 ), adapter.GetIcon( "HIGHLIGHTER_PRESETS_HL" ) );
            r_line.y += r_line.height;
        }
    ...
    {   Adapter.INTERNAL_BOX( inputrect, "" );
        ...
        GUI.DrawTexture( new Rect( r_header.x + 9, r_header.y + r_header.height - 1, r_header.width - 18, 5 ), adapter.GetIcon( "HIGHLIGHTER_PRESETS_HL" ) );
        var item = cache.GetSetByIndex(selected_category_index);
        
        
        
        var haveSource = item.presets.FirstOrDefault(s => s.source != -1);
        /*   var enn = GUI.enabled;
           GUI.enabled = ;*/
        ...
        {   var result = LocalIdInFileToObject(haveSource.source);
            if ( !result ) haveSource.source = -1;
        ...
        var SC_COMP = group_rect.height < rr.height;
        if ( SC_COMP ) rr.width -= SCROLL_W;
        GUI.BeginScrollView( group_rect, scroll, rr, false, false );
        RESET_SCROLL();
    ...
    {   var button_rect = new Rect(r_line.x + 3, r_line.y + (r_line.height - 15) / 2, r_line.width - 6, 15);
        ...
        if ( isSelected ) GUI.DrawTexture( button_rect, adapter.GetIcon( "HIGHLIGHTER_PRESETS_SELECTION" ) );
        EditorGUIUtility.AddCursorRect( r_line, MouseCursor.Link );
        ...
        EditorGUI.DrawRect( save_rect, new Color( Adapter.EditorBGColor.r, Adapter.EditorBGColor.g, Adapter.EditorBGColor.b, 0.5f ) );
    ...
    {   var presetsSelect = Hierarchy_GUI.Instance(adapter).GETPRESETS_SELECT;
    ...
    {   CHECK_CACHE();
        ...
        var w = _W__InputWindow.Init(pos, "Component", adapter, (n) =>
    ...
    {   var target = category.categories[INDEX];
        var pos = new MousePos( Event.current.mousePosition, MousePos.Type.Input_190_68, false, adapter);
        // var pos = InputData.WidnwoRect(WidnwoRectType.Full, Event.current.mousePosition, 190, 68, adapter);
        var w = _W__InputWindow.Init(pos, "Rename Set", adapter, (n) =>
        {   if (string.IsNullOrEmpty(n)) return;
            if (target != null)
            {   target.name = n;
                SaveCacheAndCreateUndo();
    ...
    static Dictionary<long, int> localIdInFIle_to_instanceId = new Dictionary<long, int>();
    ...
    {   if ( category.SetType == PRESET_TYPE.SINGLE_COMPONENT )     // Debug.Log(category.PRESET_COMPONENT_TYPE);
        {   var t = Adapter.GET_TYPE_BY_STRING(category.PRESET_COMPONENT_TYPE);
            if ( t == null )     // var pos = InputData.WidnwoRect(WidnwoRectType.Full, Event.current.mousePosition, 190, 68, adapter);
            {   var pos = new MousePos( Event.current.mousePosition, MousePos.Type.Input_190_68, false, adapter);
                var n = _W__InputWindow.Init(pos, "Warning", adapter, null, null, "'" + t + "' does not exist anymore");
                n.firstLaunch = true;
                return;
            }
    ...
    internal static bool CHECK_ADD_PRESET_TOLAST( GameObject o, Adapter _adapter )
    {   adapter = _adapter;
        CHECK_CACHE();
        var presetsSelect = Hierarchy_GUI.Instance(adapter).GETPRESETS_SELECT;
        ...
        if ( all.All( c => c.GetType().FullName != target.PRESET_COMPONENT_TYPE ) ) return false;
        return true;
    }
    internal static void TRY_ADD_PRESET_TOLAST( GameObject o, Adapter _adapter )
    {   adapter = _adapter;
        Selection.objects = new[] { o };
        source = Adapter.static_adapter.GetHierarchyObjectByInstanceID( o );
        CHECK_CACHE();
        var presetsSelect = Hierarchy_GUI.Instance(adapter).GETPRESETS_SELECT;
    ...
    {   if ( cat.SetType == PRESET_TYPE.FULL_HIERARCHY || cat.SetType == PRESET_TYPE.NOT_INITIALIZED ) return;
        ...
        {   var o = source;
            var types = HierarchyExtensions.Utilities.GetComponentFast<Component>.GetAll(o.go).Where(c => c).Select(c => c.GetType()).Distinct().ToArray();
    ...
    static void m_final_add_window( PresetSet cat, Adapter.HierarchyObject o, MousePos pos, string newJson, string presetType )
    {   var inter = cat.presets.Length + 1;
        var newName = "Preset " + inter++;
        while ( cat.presets.Any( c => c.name == newName ) ) newName = "Preset " + inter++;
        var w = _W__InputWindow.Init(pos, "New Preset", adapter, (n) =>
        {   m_fnal_add_save(cat, o, n, newJson, presetType);
        }, null, newName);
        w.firstLaunch = true;
    }
    
    static void m_fnal_add_save( PresetSet cat, Adapter.HierarchyObject o, string n, string newJson, string presetType, int? useIndex = null )
    {   if ( useIndex.HasValue ) n = cat.presets[useIndex.Value].name;
        ...
        var newpreset = new SetItem() { name = n, source = o.go ? Adapter.m_TryGetGUIDAndLocalFileIdentifier(o.go) : -1 };
    ...
    static string m_final_add_preset( PresetSet cat, Adapter.HierarchyObject o, MousePos pos, Type[] types, string presetType, int? useIndex = null )          //Action<string> final = ( presetType ) => {
    {   if ( !o.go ) return null;
        ...
        {   var compname = cat.PRESET_COMPONENT_TYPE;
            var s = compname.LastIndexOf('.');
            if ( s != -1 && s < compname.Length - 1 ) compname = compname.Substring( s + 1 );
            var n = _W__InputWindow.Init(pos, "Info", adapter, null, null, "'" + o.go.name + "' does not contain '" + compname + "'");
        ...
        {   var n2 = _W__InputWindow.Init(pos, "Info", adapter, null, null, "Such preset already exists '" + cat.presets.First(p => JSON_EQUALS(p.json, newJson)).name + "'");
    ...
    {   KeeperData data = new KeeperData();
        ...
        else
        {
            ...
            var all =  HierarchyExtensions.Utilities.GetComponentFast<Component>.GetAll( o ).Where(c => c).ToList();
        ...
        return Adapter.SERIALIZE_SINGLE( data );
    }
    ...
    {   var UNDO_TEXT = "Apply Preset";
        ...
        {   var l = o.GetComponents(type);
            ...
            KeeperData data = null;
            try
            {   data = Adapter.DESERIALIZE_SINGLE<KeeperData>( json );
            }
...
#pragma warning restore
            ...
            {   adapter.logProxy.LogWarning( "Error to load '" + type.Name + "' data" );
                return;
            }
            
            var old = data.comp_to_Type.First().Value;
            if ( old != type.FullName )     //var pos = InputData.WidnwoRect(WidnwoRectType.Full, Event.current.mousePosition, 190, 68, adapter);
            {   var pos = new MousePos( Event.current.mousePosition, MousePos.Type.Input_190_68, false, adapter);
            
                var n = _W__InputWindow.Init(pos, "Warning", adapter, null, null, "Preset manager cannot read '" + old + "' " + "type");
                n.firstLaunch = true;
                return;
            }
            ...
            var currentPos = 0;
            var records = data.field_records.First().Value.records.Values.OrderBy(r => r.index).ToArray();
            ...
            {   for ( int i = currentPos ; i < l.Length ; i++ )
                {   if ( !l[i] ) continue;
                    Undo.DestroyObjectImmediate( l[i] );
                }
            }
        }
    }
    ...
    {   var jsonData = new KeeperDataUnityJsonData();
        ...
        if ( adapter.par.PresetManagerParams.SAVE_GAMEOBJEST )
        {   var f = Adapter.GET_FIELDS_AND_VALUES(obj, obj.GetType());
            ...
            {   //var v = ((UnityEngine.Object)f[jsonData.fields_name[i]].GetValue(obj));
                ...
                {   if ( !string.IsNullOrEmpty( AssetDatabase.GetAssetPath( v ) ) )
                    ...
                    {   jsonData.fields_new_value[i] = new KeeperDataFieldValue() { FILEID = Adapter.GetLocalIdentifierInFile( v ) };
                    }
                }
    ...
    {   if ( jsonData == null ) return;
        ...
        bool wasTry = false;
        if ( jsonData.fields_name != null && adapter.par.PresetManagerParams.SAVE_GAMEOBJEST )     //  MonoBehaviour.print(obj.name);
        {   var fff = Adapter.GET_FIELDS_AND_VALUES(obj, obj.GetType(), searchVals: 4);//.ToDictionary(v=>v.Key, v=>v.Value)
            ...
            {   //var f = item.Value;
                ...
                for ( int i = 0 ; i < jsonData.fields_name.Length ; i++ )
                {
                    ...
                    if ( jsonData.fields_new_value[i] == null || jsonData.fields_new_value[i].FILEID == -1 && string.IsNullOrEmpty( jsonData.fields_new_value[i].GUID ) ) continue;
                    ...
                    // else
                    {
                        ...
                        else
                        {
                            ...
                            if ( !wasTry )
                            {   if ( localIdInFIle_to_instanceId.Count == 0 || !localIdInFIle_to_instanceId.ContainsKey( jsonData.fields_new_value[i].FILEID ) )
                                {   wasTry = true;
                                    foreach ( var c in Resources.FindObjectsOfTypeAll<Component>() )
                                    {   var localID = Adapter.GetLocalIdentifierInFile( c );
                                        if ( !localIdInFIle_to_instanceId.ContainsKey( localID ) ) localIdInFIle_to_instanceId.Add( localID, c.GetInstanceID() );
                            ...
                            if ( wasTry && !localIdInFIle_to_instanceId.ContainsKey( jsonData.fields_new_value[i].FILEID ) )
                                localIdInFIle_to_instanceId.Add( jsonData.fields_new_value[i].FILEID, -1 );
                                
                                
                            oo = localIdInFIle_to_instanceId[jsonData.fields_new_value[i].FILEID] == -1 ? null : Adapter.GET_OBJECT( localIdInFIle_to_instanceId[jsonData.fields_new_value[i].FILEID] );
                        }
    ...
    internal static void INTERNAL_RECORD_FLUSH( KeeperData source, Component comp, int index, bool writeObjectData = false )        /* RECORD */
    {   gameObject_ID = Adapter.GetLocalIdentifierInFile( comp.gameObject );
        ...
        if ( !source.field_records.ContainsKey( gameObject_ID ) ) source.field_records.Add( gameObject_ID, new KeeperDataItem(
                        writeObjectData && comp.gameObject.transform.parent ? Adapter.GetLocalIdentifierInFile( comp.gameObject.transform.parent.gameObject ) : -1,
                        writeObjectData ? comp.gameObject.transform.GetSiblingIndex() : -1,
                        writeObjectData ? comp.gameObject.name : null,
                        writeObjectData ? comp.gameObject.activeSelf : false
            ) { records = new Dictionary<long, KeeperDataUnityJsonData>() } );
        if ( !source.field_records[gameObject_ID].records.ContainsKey( comp_ID ) )
        {   var j = INTERNAL_ToJson(comp, comp.gameObject.scene);
            j.index = index;
            source.field_records[gameObject_ID].records.Add( comp_ID, j );
        }
    }
