...
class CustomModule_Example_MaterialShader : ProjectExtensions.CustomModule_Slot1 {
    ...
    public override string ToString( string assetPath, string assetGuid, int instanceId, bool isFolder, bool isMainAsset )
    {
        ...
        #else
        if ( isFolder ) return ""; //to avoid slow downing search
        var t = InternalEditorUtility.GetObjectFromInstanceID(instanceId);
        if ( t && t.GetType() != materialType ) return ""; //to avoid slow downing search
        #endif
    ...
    {   if ( !isMainAsset || isFolder ) return;
        ...
        if ( GUI.Button( drawRect, mat.shader.name, HierarchyExtensions.Styles.button ) )
        {
