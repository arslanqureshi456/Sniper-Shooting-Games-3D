...
public class PlayerWeapons : MonoBehaviour {
    ...
    public bool haveNoBullets = false;
	//ABDUL
	public Weapon[] weapons;
	//public GameObject weaponSwitchBTN;
	private int count = 0;

	public void OnResetWeapons()
	{
		for (int i = 0; i < weaponOrder.Length; i++)
		{
			weaponOrder[i].GetComponent<WeaponBehavior>().bulletsLeft = weaponOrder[i].GetComponent<WeaponBehavior>().bulletsPerClip;
		}
	}

    public void OnWeaponGone()
    {
        //Debug.Log("resetting weapons");
        for (int i = 1; i < weaponBehaviors.Length - 1; i++)
        {
            weaponBehaviors[i].haveWeapon = false;
            weaponBehaviors[i].ammo = 0;
            weaponOrder[i].SetActive(false);
        }

		PlayerPrefs.SetInt("selectedWeaponIndex", firstWeapon);
		InitGrenade();
    ...
    void Start (){
		...
		else
        {
			...
			{
				if (!PlayerPrefs.GetInt("SniperWeaponSwitch").Equals(1) && count > 1 &&
					PlayerPrefs.GetInt("SniperEquipped") > 19)
					PlayerPrefs.SetInt("selectedWeaponIndex", PlayerPrefs.GetInt("SniperEquipped") - 1);
				else
					PlayerPrefs.SetInt("selectedWeaponIndex", PlayerPrefs.GetInt("SniperEquipped"));

				if (weaponOrder[(PlayerPrefs.GetInt("selectedWeaponIndex") + 1)].GetComponent<WeaponBehavior>().haveWeapon)
					StartCoroutine(SelectWeapon((PlayerPrefs.GetInt("selectedWeaponIndex") + 1)));
				else
					StartCoroutine(SelectWeapon(20));//sniper
			}
			else
			{
				if (!PlayerPrefs.GetInt("AssaultWeaponSwitch").Equals(1) && count > 1 &&
					PlayerPrefs.GetInt("AssaultEquipped") > 0)
					PlayerPrefs.SetInt("selectedWeaponIndex", PlayerPrefs.GetInt("AssaultEquipped") - 1);
				else
        ...
        BackupWeaponBehaviorComponent.droppable = true;
		...
		BackupWeaponBehaviorComponent.addsToTotalWeaps = true;
		
		UpdateTotalWeapons();

        //Waqas, changes to fix switch button bug for prevoius unlocked weapons.
        //if(LevelSelectionNew.modeSelection.Equals(0))
        //{
        //    if ((totalWeapons > 1 && PlayerPrefs.GetInt("sniperCurrentLevel") > 1))
        //    {
	...
	void Update (){
		...
		&& !FPSWalkerComponent.holdingObject){
            ...
            //    }
            //}
            //else if (!MultiPlayerManager.Instance)
            //{
            //    GameManager.Instance.ShowAlert(false);
            //}
				...
				if(Time.timeScale > 0.0f){
					...
					|| (dropWeapon && totalWeapons != 0)){//drop weapon button pressed and player has weapons in their inventory
						if(currentWeapon < weaponOrder.Length -1){//not starting at last weapon
							...
							PlayerPrefs.SetInt("AssaultWeaponSwitch", 1);
							if (PlayerPrefs.GetInt("AssaultWeaponSwitch").Equals(1))
							{
								try
								{
									foreach (GameObject go in GameManager.Instance.coverWeaponSwitchObjects)
										if (go.activeSelf)
											go.SetActive(false);
								}
								catch { }
...
//				}
	...
	public void UpdateTotalWeapons (){
		...
		for(int i = 0; i < weaponOrder.Length; i++){//iterate through weaponOrder array and count total weapons in player's inventory
	...
	public IEnumerator SelectWeapon ( int index, bool isOffhandThrow = false, bool endOffhandThrow = false){
		...
		for (int i = 0; i < weaponOrder.Length; i++){
			...
			if (i == index){
				...
				CurrentWeaponBehaviorComponent.InitializeWeapon(weapons, PlayerPrefs.GetInt("selectedWeaponIndex"));
                if (index != grenadeWeapon && GameManager.Instance)
                    GameManager.Instance.UpdateBullets(weaponOrder[index].GetComponent<WeaponBehavior>().ammo, weaponOrder[index].GetComponent<WeaponBehavior>().bulletsLeft);
                ...
                if(UnityEngine.SceneManagement.SceneManager.GetActiveScene().name == "Game")
                    GameManager.Instance.ResetCurrentWeaponTexture(i - 1);
            }	
		}
        if (UnityEngine.SceneManagement.SceneManager.GetActiveScene().name == "Game")
        {
            GameManager.Instance.SetCurrentWeaponTexture(index - 1);
        }
    }
    ...
    {
		GrenadeWeaponBehaviorComponent.haveWeapon = true;
		if (PlayerPrefs.GetInt("Grenade") > 0)
		{
			GrenadeWeaponBehaviorComponent.maxAmmo = PlayerPrefs.GetInt("Grenade");
			GrenadeWeaponBehaviorComponent.ammo = GrenadeWeaponBehaviorComponent.maxAmmo;
		}
	}
	...
	public void InitWeapons()
	{
		...
		if(LevelSelectionNew.modeSelection == LevelSelectionNew.modeType.SNIPER)// Sniper
        {
            for (int i = 20; i < 26; i++)
			{
				...
				if (PlayerPrefs.GetInt("weapon" + (i - 1)).Equals(1))
				{
					++count;
					weaponBehaviors[i].haveWeapon = true;
					weaponBehaviors[i].ammo = weaponBehaviors[i].maxAmmo;
				}
			}

			if (count > 1)
			{
				GameManager.Instance.sniperWeaponSwitchButton.SetActive(true);
				if (!PlayerPrefs.GetInt("SniperWeaponSwitch").Equals(1) &&
					PlayerPrefs.GetInt("sniperLevelUnlocked-1").Equals(1))
				{
					foreach (GameObject go in GameManager.Instance.sniperWeaponSwitchObjects)
						go.SetActive(true);
				}
				else
                {
		...
		else if (LevelSelectionNew.modeSelection == LevelSelectionNew.modeType.ASSAULT || LevelSelectionNew.modeSelection == LevelSelectionNew.modeType.COVERSTRIKE)// Assault
        {
			for (int i = 1; i < 20; i++)
			{
				if (PlayerPrefs.GetInt("weapon" + (i - 1)).Equals(1))
			...
			if (count > 1)
            {
				GameManager.Instance.assaultWeaponSwitchButton.SetActive(true);
				GameManager.Instance.coverStrikeWeaponSwitchButton.SetActive(true);
				
				if (!PlayerPrefs.GetInt("AssaultWeaponSwitch").Equals(1))
                {
					if (PlayerPrefs.GetInt("currentLevel") != 1)
					{
						foreach (GameObject go in GameManager.Instance.assaultWeaponSwitchObjects)
							go.SetActive(true);
					}
    ...
    public void _NextWeaponButton(bool isAuto = false)
    {
		if (AudioManager.instance)
            AudioManager.instance.NormalClick();
        if(UnityEngine.SceneManagement.SceneManager.GetActiveScene().name == "Game")
            GameManager.Instance.ResetCurrentWeaponTexture(currentWeapon);
        ...
        {//drop weapon button pressed and player has weapons in their inventory
            ...
            {//not starting at last weapon
                for (int i = currentWeapon; i < weaponOrder.Length; i++)
                {
                    WeaponBehavior ThisWeaponBehavior = weaponOrder[i].GetComponent<WeaponBehavior>();
                    //cycle weapon selection manually
                    if (isAuto && !haveNoBullets && ThisWeaponBehavior.ammo <= 0 && ThisWeaponBehavior.bulletsLeft <= 0)
                        continue;
                    if ( (ThisWeaponBehavior.haveWeapon && ThisWeaponBehavior.cycleSelect
                        && i != currentWeapon && !dropWeapon)
                        //do not select backupWeapon if dropping a weapon and automatically selecting the next weapon
                            ...
                            && i != currentWeapon && i != backupWeapon && dropWeapon))
                    {
						...
						PlayerPrefs.SetInt("selectedWeaponIndex", i - 1);
						//SaveManager.Instance.state.selectedWeaponIndex = i - 1;
                    ...
                    {//reached end of list, count forwards from zero to find next weapon
                        for (int n = 0; n < weaponOrder.Length - 1; n++)
                        {
                                    ...
                                    && n != currentWeapon && n != backupWeapon && dropWeapon))
                            {
								...
								PlayerPrefs.SetInt("selectedWeaponIndex", n - 1);
								//SaveManager.Instance.state.selectedWeaponIndex = n - 1;
                                //SaveManager.Instance.Save();
                                break;
                            }
                        }
		...
		{
			PlayerPrefs.SetInt("SniperWeaponSwitch", 1);
			if (PlayerPrefs.GetInt("SniperWeaponSwitch").Equals(1))
			{
				foreach (GameObject go in GameManager.Instance.sniperWeaponSwitchObjects)
					if (go.activeSelf)
    ...
    public void _ThrowGrenadeButton()
    {
        if (AudioManager.instance)
            AudioManager.instance.NormalClick();

        InputComponent.grenadeHold = true;
        InputComponent.grenadeThrown = true;
		if (PlayerPrefs.GetInt("Grenade") > 0)
		{
            if (currentWeapon != grenadeWeapon && !displayingGrenade)
            {
                if (InputComponent.grenadeHold && !pullGrenadeState && !CameraControlComponent.rotating)
                {
                    //cancel a pulled weapon attack if using offhand weapon to prevent pulled weapon fire when switching weapons
                    ...
                    grenadeThrownState = false;
                    pullGrenadeState = true;
                    InputComponent.grenadeHold = false;

					PlayerPrefs.SetInt("Grenade", PlayerPrefs.GetInt("Grenade") - 1);
					//SaveManager.Instance.state.grenade -= 1;
					//SaveManager.Instance.Save();
    ...
    { 
        for (int i = 0; i < weaponBehaviors.Length; i++)
            if (weaponBehaviors[i].haveWeapon && weaponBehaviors[i].ammo > 0 && weaponBehaviors[i].bulletsLeft > 0)
            {
                _NextWeaponButton(true);
                return false;
            }
        haveNoBullets = true;
        return true;
