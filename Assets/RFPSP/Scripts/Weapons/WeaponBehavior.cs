...
public class WeaponBehavior : MonoBehaviour {
	...
	public float kickUp = 0.0f; // 7.0
	[Tooltip("Amount to kick view sideways when firing.")]
	public float kickSide = 0.0f; // 2.0
	[Tooltip("Amount to kick view up when firing.")]
	public float kickRoll = 0.0f; // 15.0
	...
	private float kickRollAmt = 0.0f;//amount to kick view up when firing (set in editor) // 20.0
	...
	private int sniperLevelIndex = 0;
	...
	void Start() {
        ...
        if (AutoQualityChooser.finalResult >= 1)
            showEffects = true;
        else
            showEffects = false;
        fireLastBullet = false;
        //ABDUL
        sniperLevelIndex = PlayerPrefs.GetInt("sniperCurrentLevel");
        if (LevelSelectionNew.modeSelection != LevelSelectionNew.modeType.SNIPER && LevelSelectionNew.modeSelection != LevelSelectionNew.modeType.COVERSTRIKE)
            canAutoAim = true;
		...
		if(!unarmed){
			...
			if(!PistolSprintAnim){recoveryTimeAmt = 0.4f;}else{recoveryTimeAmt = 0.2f;}
                ...
                GameManager.Instance.UpdateBullets(ammo, bulletsLeft);
        }
    }

    private int baseDamage = 0;
    public void InitializeWeapon (Weapon[] weapons, int currentIndex) {
		try
		{
			myTransform = transform;//cache transform for efficiency
			mainCamTransform = Camera.main.transform;
			...
			if (!unarmed && ((ammo > 0 || doReload) || meleeSwingDelay > 0.0f))
			{
				...
				//{
				//           if (currentIndex <= PlayerWeaponsComponent.weapons.Length)//knife
				//       {
				//           damage = (int)weapons[currentIndex]._baseDamage;
				//           reloadTime = weapons[currentIndex]._baseReloadTime;
				//           zoomFOV = weapons[currentIndex]._baseZoom;
				//           bulletsPerClip = (int)weapons[currentIndex]._baseMagzineSize;
				//           bulletsToReload = (int)weapons[currentIndex]._baseMagzineSize;
				//           maxAmmo = (int)weapons[currentIndex]._baseMagzineSize * (int)weapons[currentIndex]._baseNumberOfMagzine;
				//           fireRate = weapons[currentIndex]._baseFireRate;
				//           range = weapons[currentIndex]._baseRange;

				//           // Hiding this section for recoiling of guns
				////           if (PlayerPrefs.GetInt("selectedWeaponIndex") < 19)//Assault weaopns
				////           {
				////               shotSpread = weapons[currentIndex]._baseSpread / 2;
				////             //  kickUp = 0;//weapons[currentIndex]._baseKickUp;
				////               kickSide = 0;//weapons[currentIndex]._baseKickSide;
				////               viewClimbSide = 0;///weapons[currentIndex]._baseClimbSide;
				////useViewClimb = false;//weapons[currentIndex]._baseViewClimbOff;
				////           }
				////           else
				////           {
				////               //shotSpread = weapons[currentIndex]._baseSpread;
				////               //kickUp = weapons[currentIndex]._baseKickUp;
				////               //kickSide = weapons[currentIndex]._baseKickSide;
				////               //viewClimbSide = weapons[currentIndex]._baseClimbSide;
				////               //useViewClimb = weapons[currentIndex]._baseViewClimbOff;
				////           }
				//       }
    ...
    {
            GameManager.Instance.UpdateBullets(ammo, bulletsLeft);
            GameManager.Instance.ToggleFireButtons();
    ...
    void LateUpdate() {
        ...
        if (Time.timeScale > 0.0f && Time.deltaTime > 0.0f && Time.smoothDeltaTime > 0.0f){//allow pausing by setting timescale to 0
            ...
            rollRot = 0;
            if (WeaponPivotComponent != null && IronsightsComponent != null && CamAndWeapAnimsComponent != null)
    ...
    private float i = 0.2f;//crouch time
    private float j = 0.3f;//fire time
    private bool IsHit, IsHeadShot = false;
    private RaycastHit autoHit;
    HitBox hitbox;

	public SmoothMouseLook player;
	public Transform raycastedObject;
	public LayerMask aimAssistMask;
	public float speedToAssist;
    ...
    void Update(){
        ...
        //ABDUL
        if (LevelSelectionNew.modeSelection == LevelSelectionNew.modeType.COVERSTRIKE && isAutoFire)
        {
            if (Physics.Raycast(this.transform.root.position, transform.TransformDirection(Vector3.forward), out autoHit, range, bulletMask))
            {
                UnityEngine.Debug.DrawRay(this.transform.root.position, transform.TransformDirection(Vector3.forward) * autoHit.distance, Color.yellow);
                if (!Player.Instance.isReloading && !Player.Instance.canMove)
                {
                    if (autoHit.transform.CompareTag("HitBox"))
                    {
                        if (i > 0)
                            i -= Time.deltaTime;
                        if (j > 0)
		...
		else
        {
			...
			//if (LevelSelectionNew.modeSelection.Equals(0))
			//{
			if (Physics.Raycast(playerObj.transform.position + pos, transform.TransformDirection(Vector3.forward), out autoHit, range, bulletMask))
			{
				UnityEngine.Debug.DrawRay(playerObj.transform.position + pos, transform.TransformDirection(Vector3.forward) * autoHit.distance, Color.yellow);
				if (autoHit.collider.CompareTag("HitBox"))
				{
					hitbox = autoHit.collider.gameObject.GetComponent<HitBox>();
					if (hitbox != null && hitbox.damageMultiplyer > 1)
					{
						damage = baseDamage * 5;
						IsHeadShot = true;
					}
					...
					IsHit = true;
					if ((autoHit.transform.name.Contains("Explosive") || autoHit.transform.name.Contains("Null_2") || (hitbox && hitbox.myScript.health > 0 && PlayerPrefs.GetInt("selectedWeaponIndex") < 19)) && isAutoFire || (PlayerPrefs.GetInt("selectedWeaponIndex").Equals(26) ||
					PlayerPrefs.GetInt("selectedWeaponIndex").Equals(27)))// Snipers are not included
					{
						switch (LevelSelectionNew.modeSelection)
						{
							case LevelSelectionNew.modeType.MULTIPLAYER:
								if (autoHit.transform.GetComponent<TacticalAI.HitBox>().teamID != 1)
									try
									{
										Fire();
                                    }
							...
							default:
								if (GameManager.Instance)
									GameManager.Instance.crossHair.color = Color.red;
								try
								{
									Fire();
                                }
        ...
        if (LevelSelectionNew.modeSelection == LevelSelectionNew.modeType.ASSAULT && isAutoFire)
        {
            if (Physics.Raycast(playerObj.transform.position /*+ pos*/, transform.TransformDirection(Vector3.forward), out autoHit, range, aimAssistMask))
            {
                if (autoHit.collider.CompareTag("AutoAssist"))
                {
                    raycastedObject = autoHit.collider.transform;

                    raycastedObject.gameObject.GetComponent<Collider>().enabled = false;


                    /*NEW ROTATION MECHANICS*/
                    var rotation = Quaternion.LookRotation(raycastedObject.position - transform.position, Vector3.forward);
                    float angleX = rotation.eulerAngles.y;
                    float angleY = rotation.eulerAngles.x;
                    if (player.maximumX < 0 && player.minimumX < 0)
                    {
                        angleX = (angleX > 0) ? angleX - 360 : angleX;
                    }
                    else if (player.minimumX > 180 || player.maximumX > 180)
                    {
                    ...
                    player.rotationX = Mathf.LerpAngle(player.rotationX, angleX, speedToAssist * Time.deltaTime);
                    player.rotationY = Mathf.LerpAngle(player.rotationY, -angleY, speedToAssist * Time.deltaTime);
        ...
        if (Time.timeScale > 0.0f && Time.deltaTime > 0.0f){//allow pausing by setting timescale to 0
            ...
            if (mainCamTransform != null && myTransform != null && myTransform.parent != null)
            {
            ...
            {
                weaponLookDirection = Vector3.zero;
            }
	...
	public TargetScript[] _targetScripts;
    bool isDistaceSet = false;
	...
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	///s
	public static bool fireLastBullet = false;
    private GameObject lastEnemy;
    private void Fire(){
        if (FPSPlayerComponent.isReSpawning)
        {
            return;
        }
        

        //if (LevelSelectionNew.modeSelection.Equals(1))
        //{
        //    _targetScripts = FindObjectsOfType<TargetScript>();
        //    foreach (TargetScript ts in _targetScripts)
        //    {
        //        ts.maxDistToNoticeTarget = 9999;
        //    }
        //}
			...
			if ((canShoot && !PlayerWeaponsComponent.switching) || doReleaseFire || meleeActive){//don't allow shooting when reloading, sprinting, or switching
				...
				if(((recoveryTime + recoveryTimeAmt < Time.time || CameraControlComponent.thirdPersonActive) || doReleaseFire || meleeActive) && (startTime + readyTimeAmt < Time.time)){
					...
					if (time > nextFireTime - schedulingTime){
                        ...
                        {
                            currentBulletNumber++;
                            if(currentBulletNumber == 1)
                            {
                                GameManager.firstBulletDelegate();
                            }
                            if(currentBulletNumber == 2)
                            {
                                if(GameManager.secondBulletDelegate == null)
                                    GameManager.firstBulletDelegate();
                                else
                                    GameManager.secondBulletDelegate();
                            }
                            else
                            {
                                if(GameManager.secondBulletDelegate == null)
                        ...
                        if(showEffects)
                        {
                            ...
                            {//eject shell and perform muzzle flash if not a melee weapon
                                ...
                                if(gameObject.activeSelf)
                                {
                                    ...
                                    if (spawnShell && LevelSelectionNew.modeSelection != LevelSelectionNew.modeType.COVERSTRIKE)
                                    {
                                        try {
	...
	public IEnumerator FireOneShot (){
		//if (LevelSelectionNew.modeSelection == LevelSelectionNew.modeType.SNIPER)
		//	GameManager.Instance.PlaySniperBackgroundSound();
    ...
    TacticalAI.HealthScript healthScript;
	...
	void HitObject ( RaycastHit hit, Vector3 directionArg, bool isSecondCast = false ){
		...
		case 0://hit object
			if(hit.collider.gameObject.GetComponent<AppleFall>()){
                ...
                {
                    if (GameManager.Instance.totalEnemies - GameManager.Instance.enemyKilled == 1)
                    {
                        // Adding distance to call kill cam 
                        float dist = Vector3.Distance(hit.transform.position, this.transform.position);
#if UNITY_EDITOR
                        print("Distance : " + dist + " Hitting : " + hit.transform.name);
#endif
						if (AutoQualityChooser.finalResult != 0)
                        {
                            if (!hit.transform.name.Contains("Barrel") && ((!isAutoFire /*&& hit.collider.gameObject.GetComponent<TacticalAI.HitBox>().myScript.health > 0*/) ||
                            isAutoFire) && (Vector3.Distance(GameManager.Instance.fpsPlayer.transform.position, hit.transform.position) > 9) &&
                            LevelSelectionNew.modeSelection == LevelSelectionNew.modeType.SNIPER)
                            {
                                if (LevelSelectionNew.modeSelection == LevelSelectionNew.modeType.ASSAULT && PlayerPrefs.GetInt("currentLevel") > 5 && dist >= 12 ||
                                    LevelSelectionNew.modeSelection == LevelSelectionNew.modeType.SNIPER)
                                {
                                    GunScript[] _gunScripts = FindObjectsOfType<GunScript>();
                                    foreach (GunScript gs in _gunScripts)
                                    {
                                        gs.gameObject.SetActive(false);
                                    }
                                    FPSPlayerComponent.invulnerable = true;
                                    healthScript = hit.collider.gameObject.GetComponent<TacticalAI.HitBox>().myScript;
                                    //lastEnemy = GameObject.FindWithTag("Flesh/BadNPC");
                                    lastEnemy = hit.transform.root.gameObject;
                                    if (healthScript.health > 0)
                                    {
                                        AnimationScript _as = healthScript.gameObject.GetComponent<AnimationScript>();
                                        _as.animator.SetFloat(_as.forwardsMoveHash, 0, 0, Time.deltaTime);
                                        _as.animator.SetFloat(_as.sidewaysMoveHash, 0, 0, Time.deltaTime);
                                        NavMeshAgent[] agents = FindObjectsOfType<NavMeshAgent>();
                                        for (int i = 0; i < agents.Length; i++)
                                        {
                                            agents[i].enabled = false;
                                        }
                                        TacticalAI.AnimationScript[] sc = FindObjectsOfType<TacticalAI.AnimationScript>();
                                        for (int i = 0; i < sc.Length; i++)
                                        {
                                            sc[i].enabled = false;
                                        }
                                        healthScript.gameObject.GetComponent<NavMeshAgent>().enabled = false;
                                        healthScript.gameObject.GetComponent<TacticalAI.GunScript>().enabled = false;
                                        _as.enabled = false;
                                        firefx.clip = fireSnd;
                                        AudioManager.instance.otherAudioSource.PlayOneShot(fireSnd);
                                        FPSPlayerComponent.ChangeZoomState(false);// For Hiding Scope Image
                                        InputComponent.crouchHold = false;                                             //GetComponent<BulletEffectWrapper>().endMarker = autoHit.point;
                                        BulletFollow.enemyObject = hit.transform;
                                        BulletFollow.hit = hit;
                                        if (hit.transform.root.GetComponent<Animator>())
                                            hit.transform.root.GetComponent<Animator>().enabled = false;


                                        GameManager.Instance.ShowLastBullet(hit.point);
                                        //GetComponent<BulletEffectWrapper>().Fire();
                                        return;
                                    }
                                }
                            }
                            ...
                            hit.collider.gameObject.GetComponent<TacticalAI.HitBox>().Damage(damageAmt, hit);
                    }
                    
                        hit.collider.gameObject.GetComponent<TacticalAI.HitBox>().Damage(damageAmt, hit);
                        FPSPlayerComponent.UpdateHitTime();//used for hitmarker
                }
                else if (hit.collider.gameObject.GetComponent<Heli_Health>())
                {
                    hit.collider.gameObject.GetComponent<Heli_Health>().Damage(damageAmt);
                }
                //ABDUL
                else if (hit.collider.gameObject.GetComponent<CS_HitBox>())
                {
                    hit.collider.gameObject.GetComponent<CS_HitBox>().Damage(damageAmt);
                }
                break;
        ...
        if (!isSecondCast)
        {
            ...
            if (hit.collider.gameObject.tag == "Flesh" || hit.collider.gameObject.tag == "HitBox" && PlayerPrefs.GetInt("BloodEffect").Equals(1))
            {
                hitCount++;
                if (hitCount < 4)
		...
		if (muzzleFlash){
			...
			if(!FPSWalkerComponent.holdingBreath){
				if(muzzleLightObj && !CameraControlComponent.thirdPersonActive){
					...
					muzzleLightComponent.intensity = 2.0f;
				}
            ...
            {
				//GameObject flash = (GameObject)(Instantiate(muzzleFlashEffect, muzzleRendererComponent.transform.position, muzzleRendererComponent.transform.rotation));
				GameObject flash = (GameObject)(Instantiate(muzzleFlashEffect, muzzleFlash.position, muzzleFlash.rotation));
				Destroy(flash, 0.3f);
			}
	...
	IEnumerator Reload(){
        ...
        if(LevelSelectionNew.modeSelection == LevelSelectionNew.modeType.COVERSTRIKE)
        {
			...
			Player.Instance.canCrouch = true;
            Player.Instance.EnableReloadingCircle();
            Player.Instance.HidePanel();
        }
		...
		if(Time.timeSinceLevelLoad > 2){//prevent any reloading behavior at level start 
			...
			if((!(FPSWalkerComponent.sprintActive && (Mathf.Abs(horizontal) > 0.75f || Mathf.Abs(vertical) > 0.75f))//allow reload while walking
				...
				if(ammo > 0){//if player has no ammo in their inventory for this weapon, do not proceed with reload
                    ...
                    //FPSPlayerComponent.ChangeZoomState(false);
					...
					if((bulletsToReload != bulletsPerClip && bulletsReloaded > 0) || bulletsToReload == bulletsPerClip){
                        ...
                        if (LevelSelectionNew.modeSelection == LevelSelectionNew.modeType.COVERSTRIKE)
                        {
							...
							Player.Instance.ShowPanel();
                            Player.Instance.DisableReloadingCircle();
                        }
                    }
