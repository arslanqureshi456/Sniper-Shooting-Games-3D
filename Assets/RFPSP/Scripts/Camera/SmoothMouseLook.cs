...
public class SmoothMouseLook : MonoBehaviour {
	...
	void Start(){
        ...
        //if (LevelSelectionNew.modeSelection != LevelSelectionNew.modeType.COVERSTRIKE || LevelSelectionNew.modeSelection != LevelSelectionNew.modeType.SNIPER)
        //    AimAssist.Instance.inputs = InputComponent;
    ...
    void Update(){
		...
		if(Time.timeScale > 0.0f && Time.smoothDeltaTime > 0.0f){//allow pausing by setting timescale to 0
			...
			if(!thirdPerson){
				...
				//if (canAutoAim && AimAssist.Instance.isAimingActive)
    //            {
    //                inputY = recoilX = recoilY = rotationX = rotationY = 0;
    //                originalRotation = AimAssist.Instance.AssistRot;
    ...
    public void ForceLookAt(Vector3 point)
    {
        ...
        transform.GetChild(0).GetComponent<CameraControl>().ForceLookAt(point);
#if UNITY_EDITOR
        Debug.Log("Look1 : " + point);
# endif
    }
