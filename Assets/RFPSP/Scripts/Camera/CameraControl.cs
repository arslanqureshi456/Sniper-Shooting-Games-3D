...
//	public bool drawCamSphere;
	...
	void Update (){
		...
		if (Time.timeScale > 0 && Time.deltaTime > 0 && Time.smoothDeltaTime > 0 && Time.time > 0.0f /*&& PlayerWeaponsComponent != null && InputComponent != null
			&& FPSPlayerComponent != null && playerObjTransform != null && weaponObj != null && MouseLookComponent != null && GunSwayComponent != null && FPSWalkerComponent != null
			&& IronsightsComponent != null && CurrentWeaponBehaviorComponent != null*/)
		{//allow pausing by setting timescale to 0
			...
			if (CurrentWeaponBehaviorComponent != null)
			{
			...
			if(!thirdPersonActive){
                ...
                if (LevelSelectionNew.modeSelection == LevelSelectionNew.modeType.COVERSTRIKE)
                    targetPos = new Vector3(targetPos.x, targetPos.y + ((FPSWalkerComponent.standingCapsuleheight - FPSWalkerComponent.capsule.height) * 0.4750002f), targetPos.z);
            }
			...
			if(thirdPersonActive){
                ...
                //if(LevelSelectionNew.modeSelection == LevelSelectionNew.modeType.COVERSTRIKE)
                //{
                //    camRotation = Quaternion.Slerp(camRotation, Quaternion.Euler(-vertical, horizontal, 0.0f), sensitivity);
                //}
                //else
    ...
    public void ForceLookAt(Vector3 point)
    {
        ...
        tempCamAngles = transform.rotation;
#if UNITY_EDITOR
        Debug.Log("Look2 : " + point);
# endif
    }
