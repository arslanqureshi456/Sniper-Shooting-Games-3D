    ...
    IEnumerator Detonate() {
		...
		try
		{
			...
			for (int i = 0; i < hitColliders.Length; i++)
			{
					...
					if (explosionDamageAmt >= 1 && hit.collider == hitCollider)
					{
						...
						switch (hitCollider.gameObject.layer)
						{
							...
							case 0://hit object
								...
								{
									if (LevelSelectionNew.modeSelection == LevelSelectionNew.modeType.COVERSTRIKE && hit.collider.gameObject.GetComponent<CS_HitBox>()._enemy.HP <= explosionDamageAmt)
									{
										if (transform.name.Contains("Barrel"))
											GameManager.Instance.objectiveStats[(int)AssaultLevels.LevelConidtions.isBarrelExplosion]++;
										else
											GameManager.Instance.objectiveStats[(int)AssaultLevels.LevelConidtions.isGrenadeKill]++;
									}
									hit.collider.gameObject.GetComponent<CS_HitBox>().Damage(explosionDamageAmt);
								}
								else if (hitCollider.gameObject.GetComponent<TacticalAI.HitBox>())
								{
									hitCollider.gameObject.GetComponent<TacticalAI.HitBox>().damageMultiplyer = 1; // For no headshot
									if (LevelSelectionNew.modeSelection == LevelSelectionNew.modeType.ASSAULT && hit.collider.gameObject.GetComponent<TacticalAI.HitBox>().myScript.health <= explosionDamageAmt)
											if (transform.name.Contains("Barrel"))
												GameManager.Instance.objectiveStats[(int)AssaultLevels.LevelConidtions.isBarrelExplosion]++;
											else
												GameManager.Instance.objectiveStats[(int)AssaultLevels.LevelConidtions.isGrenadeKill]++;
										hit.collider.gameObject.GetComponent<TacticalAI.HitBox>().Damage(explosionDamageAmt, hit);
								}
								break;
				...
				{//if all objects have been damaged by blast, disable collider (it was needed for the line cast above)
					if (objectPoolIndex == 0)
					{
