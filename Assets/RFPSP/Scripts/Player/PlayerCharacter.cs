...
public class PlayerCharacter : MonoBehaviour {
	//ABDUL
	//public Vector3 leftAngle, rightAngle;
	...
	//ABDUL
	[HideInInspector] public FPSRigidBodyWalker walkerComponent;
	...
	void Start () {
		...
		if(anim.GetBoneTransform(HumanBodyBones.Head)){
			...
			neckBone = anim.GetBoneTransform(HumanBodyBones.UpperChest);
		}
        else{
#if UNITY_EDITOR
	...
	void Update () {
		...
		if(anim.isInitialized && fpBodyAnim.isInitialized && Time.timeScale > 0.0f){
			...
			if(!CameraControlComponent.thirdPersonActive && displayVisibleBody){
                ...
                {
                    tempBodyAngles = new Vector3(0f, bodyAngAmt + swimFireAngAmt + playerTransform.eulerAngles.y + 90f, 0.0f);
                }
                else
                {
				...
				//ABDUL
				foreach (SkinnedMeshRenderer sMesh in fpBodySkinnedMeshes)
				{
					sMesh.shadowCastingMode = UnityEngine.Rendering.ShadowCastingMode.Off;
					sMesh.enabled = false;
				}
			}
			...
			walkerComponent.sprintActive = true;
							...
							if(!walkerComponent.grounded){
								...
								if(WeaponBehaviorComponent.tpWeaponAnimType == 5){//use default torso anim for rifle weapons, but use torso layer for other weapon types
									if(walkerComponent.crouched && proneTransition == 0){//use torso layer when crouched or transitioning to or from prone
										torsoLayerWeight = Mathf.MoveTowards(torsoLayerWeight, 0.5f, Time.deltaTime * 3f);//ABDUL
									}
									else{
	...
	void LateUpdate () {//use LateUpdate() to actually set the bone angles because they are overwitten if set in Update()
		//ABDUL
   //     if (FPSPlayerComponent.InputComponent.fireHold && neckBone)
   //     {
   //         Ray ray = Camera.main.ViewportPointToRay(new Vector3(0.5f, 0.5f, 0));
   //         Vector3 point = ray.origin + (ray.direction * 5f);
   //         neckBone.LookAt(point);
			//anim.GetBoneTransform(HumanBodyBones.LeftShoulder).localEulerAngles += leftAngle;
			//anim.GetBoneTransform(HumanBodyBones.RightShoulder).localEulerAngles += rightAngle;
   //     }
	...
	public void Footstep()
	{
		...
		print("Player Steps");
#endif
		footstepSource.PlayOneShot(footstep[Random.Range(0,footstep.Length)]);
