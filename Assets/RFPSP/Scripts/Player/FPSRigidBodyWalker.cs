...
public class FPSRigidBodyWalker : MonoBehaviour {
	...
	public float antiBunnyHopFactor = 1.0f;
	[HideInInspector]
    ...
    void FixedUpdate(){
		...
		if(Time.timeScale > 0){//allow pausing by setting timescale to 0
			...
			if (grounded) {
				...
				if(InputComponent && WeaponBehaviorComponent && FPSPlayerComponent
					&& ((sprintActive && InputComponent.fireHold)//cancel sprint if fire button is pressed
				...
				else if(InputComponent.sprintHold)//ABDUL
                {
					cancelSprint = false;
					sprintActive = true;
				}
