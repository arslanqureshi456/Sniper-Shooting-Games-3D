...
public class InputControl : MonoBehaviour {
	...
	void Update () {
		...
		if(FPSPlayerComponent && !FPSPlayerComponent.restarting){
            ...
            if (LevelSelectionNew.modeSelection == LevelSelectionNew.modeType.SNIPER ||
                ...
                moveInput = new Vector2(_variableJoystick.Horizontal, _variableJoystick.Vertical);
            ...
            if (ControlFreak2.CF2Input.GetAxisRaw("Xbox R Trigger") > 0.1f || ControlFreak2.CF2Input.GetButton("Fire"))
            {
                if (LevelSelectionNew.modeSelection == LevelSelectionNew.modeType.COVERSTRIKE)
                {
                    if (!SaveManager.Instance.state.autoShoot.Equals(1))
                        StartCoroutine(EnableFire(0.15f));
                }
                else
                {
					if(SaveManager.Instance.state.autoShoot.Equals(0))
                    {
            ...
            if (LevelSelectionNew.modeSelection == LevelSelectionNew.modeType.COVERSTRIKE)
            {
                if (fireHold && !Player.Instance.isReloading)
                {
                    FPSPlayerComponent.IronsightsComponent.defaultFov = 20f;//25
                }
                else
                {
                    FPSPlayerComponent.IronsightsComponent.defaultFov = 45f;
                }
            }
    ...
    private IEnumerator EnableFire(float delay)
    {
